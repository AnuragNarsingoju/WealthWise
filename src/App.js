 ...
 useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        if (user) {
          setMail(user.email);
          console.log("AFADF : ",auth.currentUser.email);
        } else {
          // setMail('');
          // await auth.signOut();
        }
  ...
  return (
    <div className="App">
      <Routes>
        ...
        <Route path="*" element={auth.currentUser.email  ? <PageNotFound /> : <Login user1={setLog} email={setMail} />} />
        <Route path="/home" element={auth.currentUser.email ? <Home mail={mail} /> : <Login user1={setLog} email={setMail} />} />
        <Route path="/foam" element={auth.currentUser.email ? <Psinfo mail={mail} /> : <Login user1={setLog} email={setMail} />} />
        <Route path="/chatbot" element={ auth.currentUser.email ? <ChatBot mail={mail} /> : <Login user1={setLog} email={setMail} />} />
        <Route path="/fileupload" element={ auth.currentUser.email === 'anuragnarsingoju@gmail.com' ? <FileUpload /> :  <PageNotFound /> } />
        ...
        <Route path="/personal-MF" element={ auth.currentUser.email ? <InvestmentRecommendationForm /> : <Login user1={setLog} email={setMail} />} />
      </Routes>
    </div>
  );
};
